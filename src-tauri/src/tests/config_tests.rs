...
fn test_override_split_roundtrip() {
    ...
    config.frontend_data = serde_json::json!({
        "theme": "dark",
        "favorites": [{"path": "C:\\pics\\a.jpg", "name": "a.jpg"}],
    ...
    assert_eq!(
        loaded.frontend_data["favorites"][0]["path"],
        "C:\\pics\\a.jpg"
    );
...
fn test_override_single_file_layout() {
    ...
    config.frontend_data =
        serde_json::json!({"theme": "dark", "favorites": [{"path": "C:\\pics\\a.jpg"}]});
    ...
    let main: serde_json::Value =
        read_json_file(&dir.join("quivit_config.json")).unwrap();
    assert_eq!(
        main["frontend_data"]["favorites"][0]["path"],
        "C:\\pics\\a.jpg"
    );
