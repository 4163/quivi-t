...
KIRO_SESSIONS_DIR = os.path.expanduser(r'C:\Users\x4163\.kiro\sessions')
        ...
        for s_id in os.listdir(wp):
            ...
            if os.path.isfile(sess_file):
                try:
                    ...
                    # Filter by workspacePaths if available
                    ...
                    if not is_quivit and paths:
                        continue # Skip non-QuiviT sessions if paths are specified
...
def cmd_list():
    ...
    print(f"=== Kiro Sessions (Project: QuiviT) ===")
    if not sessions:
...
def cmd_search(keyword):
    ...
    for s in sessions:
        ...
        if not os.path.isfile(msgs_file): continue
