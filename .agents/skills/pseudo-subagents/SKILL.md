...
1. **Identify Targets:** Break the research goal into discrete, non-overlapping domains (e.g., pipeline A, pipeline B, IPC layer).
...
3. **Wait & Gather:** Wait for the background `run_command` tasks to complete. Do NOT poll. The system will notify you when they finish.
...
1. **Be specific about boundaries:** Tell the subagent exactly which files or directories to read (e.g., "Thoroughly read `core.js` and `actions.js`.").
2. **Demand factual mapping:** Instruct the subagent to trace, map, or analyze rather than solve. (e.g., "Map out the state mutation pipeline. Do not suggest fixes.").
3. **Specify the exact output path:** Explicitly tell the subagent where to write the file so you can read it later (e.g., "Write your findings directly to `.agents/scratch/state_machine.md`.").
