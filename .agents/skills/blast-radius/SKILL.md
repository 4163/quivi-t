...
- **Viewer DOM & rendering pool** (`src/js/viewer/viewerRender.js`, `viewerPipelines.js`, `src/js/pipelines/`): changed element IDs (`viewer-img-wrapper`, `viewer-bridge-layer`), active or bridge image pool classes, frozen `--bridge-*` transform props, or canvas render-ready attributes break viewport display and telemetry probes in `e2e/replay-diagnostics/probes/viewerPipelineProbe.js`.
...
| `src-tauri/src/commands/network.rs`, `src/js/urlLoader.js`, `extractors/` | `cargo test --manifest-path src-tauri/Cargo.toml network::tests` and `npx mocha mocha/urlLoader.test.js` | ~1.5s |
