...
- **Cross-window state** (`localStorage`, theme, preview payload, configured Library location): changed shapes or keys break secondary windows that read what the primary writes. A Library move must rebind each process watcher and refresh its cached path.
