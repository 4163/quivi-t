...
> - `src/js/services/pipelines/glRuntime.js` (345 lines) — **note:** requested path `src/js/viewer/pipelines/glRuntime.js` does not exist; actual module is `src/js/services/pipelines/glRuntime.js`, imported by `viewerPipelines.js:5` as `createGlRuntime`
...
| 3 | `createViewerRenderer(viewportState, cb)` — `viewer.js:21-24` | `cb = (img) => img ? pipelines.setSource(img) : pipelines.clear()` — **the single active/bridge → pipeline handoff**. Telemetry hook: wrap this callback |
| 4 | `createViewerGestures(viewportState)` — `viewer.js:25` | Cursor auto-hide only; not in paint path |
| 5 | `ResizeObserver` — `viewer.js:29-38` | On `width>0 && height>0`: `viewportState.handleViewportResize(w,h)` then `pipelines.forceRender()`. No debounce here — fires per RO entry |
...
## 2. Image loading lifecycle — `viewerRender.js`
...
## 3. Active / bridge image handoffs — `viewerRender.js:221-248, 58-67, 184-202, 217-219`
...
| `_attachLoadHandler` | `149-157` | `load` → `if (el!==img) return; if (!el.src) return; _stopLoadingAnimation(); add .active; _syncActiveImage(...)` — late-load path when `loadTarget` assigned src but decode raced ahead |
...
Drawing-buffer size always tracks CSS viewport size at draw time — resize is implicit, no separate resize API.
...
`viewportState.subscribe`: `imgWrapper.style.transform = getTransform()`; `--zoom-scale = getScale()`; `Statusbar.setZoom(scale)` if `img.src`. Pure compositor path — no re-upload.

---

## 5. Texture upload — `blobImage.js` + `glRuntime.js:170-209, 145-168`
...
- `getCleanImageCrop(src,sx,sy,sw,sh)` (`48-52`): uncached `fetch→blob→createImageBitmap(blob,sx,sy,sw,sh)` — currently no callers in the five files (reserved for tiled/crop uploads).
...
3. `cleanImg = await getCleanImage(imgElement.src)` — **the only await in the static path**; `catch → return null`.
...
## 6. WebGL shader execution — `glRuntime.js` + `viewerPipelines.js:100-211`
...
`render()` resolves `true` on drawn, `null` on any bail (inactive, no programs, bad dims, stale token, fetch/bitmap/tex failure, missing input). Callers treat `null` as silent no-paint (no retry — next geometry/state change re-triggers).
...
## 10. Surgical hook points for diagnostic telemetry (H1–H16)

Conventions: `t0 = performance.now()` at entry; log `{evt, src, gen, ms}`; prefer `performance.mark/measure` + a single `debugLog` funnel so hooks add ~0 overhead when disabled. All hooks are additive — no behavior change.
...
**H5 — Activate / bridge handoff.** `_activatePoolNode:221-248`: log `{outgoing:!!, bridgeMs}` — timestamp at `.bridge` add, second timestamp in double-rAF body (`229-235`); log `_cancelRetiringNode:58` hits (interrupted bridges = fast nav). Pair H1→H5 = **state-to-active latency**; H4→H5 = **decode-to-paint-ready (DOM)**.
