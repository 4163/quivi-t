...
- `urlLoader.js` owns remote extractor validation, URL import, and gallery download coordination without owning DOM. `main/urlOverlay.js` owns `#url-overlay`; `filepanel/libraryStore.js` owns Library tree data and persistence while `filePanel.js` owns its rendering; `viewerAudio.js` owns `#viewer-audio`, per-file volume and mute state, and viewport audio controls; `viewerPipelines.js` owns overlay canvases and GL texture streaming for images and video.
