...
- [ ] Collapse jump scoring to the contract tiers with the art fast path kept and documented as performance, not semantics. Remove the numeric archaeology. Proof: targeted mocha plus direct-link jumps to chapter copies in the running app. Deviation affects replacement jump tests.
- [ ] Extract one shared sidecar matcher used by both the jump scan and the cleanup link phase. Proof: `node --check` plus unchanged mocha count before replacement tests land. Deviation affects both test-rewrite items.
- [ ] Remove the duplicate export keys and fix any error-label cosmetics touched along the way. Nothing else in the export surface changes. Proof: `node --check` plus full mocha green.
- [ ] Re-derive extractor adoption against the unified core. K MANGA absorbed list and MangaDex art duplicates must resolve with zero extractor changes; if either needs a change, the core unification missed something and the item stops here. Proof: episode and art mocha coverage rewritten from scratch. Deviation affects the README item.
