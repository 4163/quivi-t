...
- **File Deletion Fallbacks:** When an active archive or folder is deleted while viewing, the user is properly booted back. `..`/startup recovery walks up via `parentOf()` to the nearest existing ancestor, falling back to the Drives view at the root.
