...
Do **not** add:
- features, bug fixes, or UX polish — those go in `implemented.md` and, if user-facing, `README.md`
...
10. Add entry to `.agents/sessions-index.md`. If multiple slices were done in the same session, and slice difference issignificant enough, prefer adding a new entry over modifying.
11. Repeat static and runtime checks as needed.
12. Leave the repo ready for the push pipeline: intended diff only, nothing extra staged, no secrets, no private paths.
