...
3. **SVG Exclusion from Upscaling Filters:** SVGs are vectors. The browser rasterizes them at the exact display resolution, so running Anime4K (upscaling/enhancement) or Lanczos (resampling) on a rasterized SVG is pointless and would degrade quality compared to native rendering. Both static and animated SVGs must skip Anime4K and Lanczos. CRT and other visual-effect filters that don't attempt to "improve" resolution remain valid on SVGs. Implementation: `_resolveActiveFilter` and `getEffectiveScaling` (or the pump gate) should check for SVG source and exclude these two pipelines.
...
- **Slice 2: Live Blob Cache:** Added a dedicated caching mechanism and asynchronous method (`getLiveImage`) to fetch same-origin Blob URLs and decode them into `HTMLImageElement` (`new Image()`) instances. Added an explicit `evictLiveBlobCache` function to properly release memory (`src = ''` and object URL revocation). Crucially, left the highly optimized `getCleanImage` (which relies on `createImageBitmap` and `ImageBitmap.close()`) entirely untouched. This isolates animated playback constraints from still-image performance, paving the way for the animation pump to bypass cross-origin WebGL taint rules.

- **Slice 3: The Frame Pump & WebGL Upload:** Modified `viewerPipelines.js` to allow filters on animated images via `_syncLivePump()`, leveraging `requestAnimationFrame` at 60fps. Refactored the pump to use `ImageDecoder` (WebCodecs) for raster formats (GIF/APNG/WebP) to natively handle disposal/compositing and avoid cross-origin taint, uploading fully composited `VideoFrame`s directly to WebGL. Implemented a fallback pump for animated SVGs using a hidden DOM `<img>` (`opacity: 0.001`) in the viewport. Excluded SVGs from Anime4K and Lanczos to preserve native vector rendering.

- **Slice 4: WebGL Lanczos:** Implemented a custom 1-pass WebGL Lanczos 3 shader (lanczosWebGL.js) specifically for the animated frame pump. Updated  iewerPipelines.js to route animated Lanczos scaling requests directly to glRuntime utilizing this new shader instead of the Pica pipeline. Modified  iewerMath.js to lift the isAnimated bilinear fallback restriction for Lanczos scaling, allowing the WebGL shader to engage seamlessly for animated formats while preserving bilinear fallback for vector SVGs.

- **Slice 5: Custom GIF Byte Scanner for Accurate Animation Detection:** Replaced the naive `NETSCAPE2.0` string scan in `formats.rs` with a robust byte scanner that counts `0x2C` Image Descriptor frames up to 8 KiB. Centralized IPC by returning an `AnimationInfo` struct (`{ is_animated: bool, no_loop: bool }`). Updated the DOM `<img>` reset logic in `viewerRender.js` to strictly apply only when `state.noLoop === true`, eliminating heavy node re-mounts on looping GIFs. Clamped the WebCodecs frame pump in `viewerPipelines.js` to prevent no-loop GIFs from looping when WebGL filters or Lanczos scaling are active.
