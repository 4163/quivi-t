...
- **Slice 3: The Frame Pump & WebGL Upload:** Modified `viewerPipelines.js` to allow filters to resolve on animated images. Introduced `_syncLivePump()` which leverages `requestAnimationFrame` to tick at 60fps when an animated image is active alongside a filter or Lanczos scaling. At each tick, the pump evaluates the frame, optionally scales it down via an in-memory staging canvas (applying the `ANIME4K_MAX_EDGE` cap), and calls a new direct upload method `updateSource(uploadSource)` on the `glRuntime`. The WebGL pipeline's `render()` method was extended with a `skipUpload` flag to allow the pump to draw the uploaded texture using standard pipeline geometry, cleanly bypassing the still-image `getCleanImage` fetch path.
